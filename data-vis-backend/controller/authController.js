...
const authController = {
    ...
    signIn : async(req , res) => {
        ...
        const user = await dbService.findOne(UserData , {email : email , password : passwordHash})

        if(!user){
            return new ApiResponse(res).customError(400 , {
                message : 'please check email and password and make sure you have signed up before signing in!'
            })   
        }
        ...
        if(user_cookie){
            ...
            res.cookie('access_token', user_cookie.access_token, { maxAge: 900000, httpOnly: true });
            return new ApiResponse(res).successful("user signup successful" , {token : user_cookie.access_token})
        }
        ...
        try{
            ...
            res.cookie('access_token', token , { maxAge: 900000, httpOnly: true , sameSite : "strict"});

            return new ApiResponse(res).successful("user signin successful" , {token : token})
        }catch(err){
            return new ApiResponse(res).customError(400 , {
                err: err.toString()
            })
